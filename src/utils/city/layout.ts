...
function skylineLayout(
  children: ProcessedNodeData[],
  containerWidth: number
): LayoutResult {
  const skyline: SkylineSegment[] = [{ // Wolne miejsce nad "ziemią". lista wolnych przestrzeni, coś w stylu skyline problem.
    x: SPACING,
    z: SPACING,
...
function updateSkyline(
  ...
  for (let j = 0; j < newSegments.length; j++) {
    if (skyline.length > 0) {
      ...
      if (Math.abs(last.z - current.z) < 0.1 && 
          Math.abs(last.x + last.width - current.x) < 0.1) {
...
export function processNode(node: CityNode): ProcessedNodeData {
  ...
  if (node.height !== undefined && node.width !== undefined) {
    ...
    const depth = width
    const height = node.height < 0.1 ? 0.1 * BUILDING_HEIGHT_SCALE : node.height * BUILDING_HEIGHT_SCALE
